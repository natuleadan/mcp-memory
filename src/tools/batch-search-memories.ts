...
export function registerBatchSearchMemoriesTool(server: McpServer) {
  server.tool(
    ...
    {
      queries: z.string().describe('Queries separated by newlines (max 5), e.g. "git rules\\npnpm usage\\nMCP workflow"'),
    ...
    async ({ queries: rawQueries, limit_per_query, type }) => {
      try {
            ...
            return {
              ...
              results: rows
                ...
                .map(r => ({
                  ...
                  type: r.type,
                  name: r.name,
                  excerpt: r.body.slice(0, 300).replace(/\n/g, ' ') + (r.body.length > 300 ? '…' : ''),
        ...
        const formatted = searchResults.map(({ query, results }) => {
          ...
          const lines = results.map(r => `  - [${r.id}] \`${r.type}\` **${r.name}** (${r.updated_at})\n    ${r.excerpt}`)
