  ...
  server.tool(
    ...
    'Smart context loader: given a task description, automatically retrieves the most relevant feedback rules, project state, and references — without loading unrelated memories. Ideal replacement for list_memories("all") at task start.',
    {
      task: z
        .string()
        .describe(
          'Description of the task you are about to perform (e.g. "commit changes to mcp-memory repo")'
        ),
      limit: z.coerce
        .number()
        .default(5)
        .describe('Max relevant memories to return per category (default: 5)'),
    },
    async ({ task, limit }) => {
      try {
        ...
        const format = (rows: typeof coreRows, truncate = 0) =>
          rows
            .map((r) => {
              const body =
                truncate && r.body.length > truncate ? r.body.slice(0, truncate) + '…' : r.body
