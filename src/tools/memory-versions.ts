  ...
  server.tool(
    ...
    async ({ name }) => {
      try {
        ...
        if (!rows.length) {
          return {
            content: [{ type: 'text', text: `No version history found for memory "${name}".` }],
          }
        }
        ...
        return {
          content: [
            {
              type: 'text',
              text: `History for **${name}** (${rows.length} versions):\n\n${formatted.join('\n\n---\n\n')}`,
            },
          ],
        }
      } catch {
        return {
          content: [
            {
              type: 'text',
              text: `No version history available. The memory_versions table does not exist yet — version tracking is recorded automatically when upsert_memory detects a change.`,
            },
          ],
        }
      }
