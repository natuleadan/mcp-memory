    ...
    async ({ type, name, body, tags }) => {
      try {
        ...
        const existing = (await table.query().where(`name = '${name}'`).toArray()) as Array<{
          id: string; type: string; body: string; tags: string; created_at: string; version?: number
        }>
        ...
        if (existing.length) {
          ...
          await table.add([{ id, type, name, body, tags: normalizedTags, created_at, updated_at: now, version: nextVersion, vector }])
          return { content: [{ type: 'text', text: `Memory "${name}" updated to v${nextVersion} (id: ${id}).` }] }
        }

        const id = randomUUID()
        await table.add([{ id, type, name, body, tags: normalizedTags, created_at: now, updated_at: now, version: 1, vector }])
